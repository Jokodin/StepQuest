	...
	static async simulateBattle(monsters) {
		...
		while (iterationCount < MAX_ITERATIONS) {
			...
			if (player.attackCooldown <= 0) {
				...
				if (targetMonster) {
					// Calculate damage with random roll between 1 and damage stat, then multiply by attack power
					const baseDamage = (Math.random() * playerDamage) + 1;
					...
					battleLog.logs.push({
						...
						displayText: `You hit ${targetMonster.id} for ${totalDamage.toFixed(1)} damage! (${targetMonster.currentHp.toFixed(1)}/${targetMonster.maxHp} HP)`
					});
			...
			initializedMonsters.forEach(monster => {
				if (monster.currentHp > 0) {
					...
					if (monster.attackCooldown <= 0) {
						// Calculate monster damage with random roll between 1 and base damage
						const monsterDamage = Math.max(1, (Math.random() * monster.baseDamage) + 1);
						...
						console.log('Monster attack debug:', {
							monsterDamage,
							playerArmor,
							mitigatedDamage,
						...
						battleLog.logs.push({
							...
							timestamp: currentTime,
							displayText: `${monster.id} hits you for ${mitigatedDamage.toFixed(1)} damage! (${player.currentHealth.toFixed(1)}/${playerHealth} HP)`
						});
